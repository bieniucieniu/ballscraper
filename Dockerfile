...
RUN ./amper task :server:resolveDependenciesLinuxX64

# 3. Copy the source code
COPY server/src ./server/src

# 4. Run the final build task
RUN ./amper task :server:linkLinuxX64Release
...
COPY --from=build /app/build/tasks/_server_linkLinuxX64Release/server.kexe ./server
